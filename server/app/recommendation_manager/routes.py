...
from app.user_manager import SERVER_URL
...
async def get_recommendations(
    ...
    logging.info(
        f"User authentication took {time.perf_counter() - start:.3f} seconds.")
    ...
    logging.info(
        f"Color parsing took {time.perf_counter() - start:.3f} seconds.")
    ...
    logging.info(
        f"Fetching items took {time.perf_counter() - start:.3f} seconds.")
    # === Evaluation Block ===
    start = time.perf_counter()
    results = {}
    location = True if lat and lon else False
    temp, weather = get_weather_at_time_by_coords(
        lat, lon, target_time) if location and target_time else (None, None)
    ...
    def evaluate_item(item: ClothingItem):
        ...
        if other_color and palette_type:
            color_score = ColorRecommendationStrategy().evaluate(
                item, other_color, palette_type)
        ...
        if not all_fields_filled:
            ...
            if location and target_time and other_color and palette_type:
                score = ColorWeatherStrategy().evaluate(
                    item, other_color, palette_type, temp, weather)
                item_results["final_match"] = {
                    "type": "color_weather_match", "result": score}

        if all_fields_filled:
            avg_score = AverageRecommendationStrategy().evaluate(
                item, temp, weather, other_color, palette_type, event)
            item_results["final_match"] = {
        ...
        return item.id, {
            ...
            "image": f"{SERVER_URL}/uploads/{item.filename}",
            **item_results
        }
    ...
    for top in grouped_items.get("tops", []):
        for bottom in grouped_items.get("bottoms", []):
            ...
            avg = average_score(*outfit_items)
    ...
    logging.info(
        f"Outfit generation took {time.perf_counter() - start:.3f} seconds.")
    logging.info(f"{outfits_detail}")
    total_duration = time.perf_counter() - start_total
    logging.info(
        f"\u23f3Request to /recommendations processed in {total_duration:.3f} seconds.")
    if location:
        return {
