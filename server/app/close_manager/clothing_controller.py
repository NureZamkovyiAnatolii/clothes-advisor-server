...
def add_clothing_item_to_db(
    ...
    existing_item = db.query(ClothingItem).filter(ClothingItem.filename == filename).first()
    if existing_item:
        raise HTTPException(status_code=400, detail="Clothing item with this filename already exists.")
